...
- **`app_started` 事件量会显著下降**，可以用来验证修复效果（预期 `app_session / app_started` 回到 1.2.0 之前的 **1.4~1.5** 区间）。但**降幅不等于后台唤醒量**，原因见下一条。
- **同时修掉一处反向漏报**：唤醒后的进程会以 cached 状态留存一段时间，用户在这个窗口里打开 app 时 `onCreate` 不再执行，旧写法整次启动都不上报。修复后这些启动开始计入，与被砍掉的唤醒量方向相反，**两者在数据里无法分离**——所以降幅只能当方向性指标，不能拿去精确估计唤醒量。
