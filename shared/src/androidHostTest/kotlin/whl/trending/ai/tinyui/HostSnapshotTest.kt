...
class HostSnapshotTest {
    ...
    fun hostMatchesTheSnapshotOfItsHostVersion() {
        ...
        if (System.getProperty("tinyui.updateHostSnapshot") != null) {
            ...
            file.writeText(HostSnapshot.render(TrendingTinyUI.host, HOST_VERSION))
            return
        }
        ...
        assertEquals(emptyList(), HostSnapshot.problems(file.readText(), TrendingTinyUI.host, HOST_VERSION), "宿主契约与 ${file.path} 不一致；$hint")
    }
}
