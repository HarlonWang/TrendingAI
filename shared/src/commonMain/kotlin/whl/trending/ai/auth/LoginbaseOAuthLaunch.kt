 ...
 * Activity，重建期间新旧两个 composition 短暂并存、同时订阅，**同一次投递会被消费两次**：
 * 终态埋点成对翻倍，`consumePendingSource()` 这类一次性读取也会被跑两遍。
 *
 * 判据是「同一次投递」而不是「同一个 flow」：同一条 flow 内用户可以真的取消两次，
 * 按 flow 去重会把第二次真实取消吞掉。靠**每次发起授权时重置**做到这一点——
 * `Cancelled` 是 object 单例，跨轮次的引用相等不能说明什么，重置后才可信。
 */
