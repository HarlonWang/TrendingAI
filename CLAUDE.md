...
**订阅页的标题、副标题、权益清单、CTA、退款说明、致谢、失败提示以 `/api/app-config` 的 `pro_paywall` 为准，页面包（`../trendingai-tinyui` 的 `i18n/`）的文案只是从未拉到配置时的兜底默认。改这些文案先改服务端（`github-ai-trending-api/src/lib/pro-paywall.js`）；订阅页新增任何文字位置，必须先进服务端契约，禁止只在页面包加键。** 权益行的图标由服务端 key 映射（页面包里的映射表），认不出的 key 用通用图标，加新行不需要先发版。订阅页是 TinyUI 页，拉价、文案、Pro 态、下单、埋点都在页面包里，本仓只接 `tinyui/TrendingTinyUI.kt` 的宿主接口。方案标签（年付 / 月付等）与价格仍在原处；单功能锁定提示、账户页一句话升级提示不受此限。取舍见后端 `docs/pro-paywall.md`。

**客户端任何文案位置（strings、弹窗、菜单尾注、占位符、Pro 闸提示）都不得出现费率、每日授予或档位数字；要提就写「详见账户页额度说明」，数字只在服务端 `quota_help` 一处。** 账户页额度说明（额度卡 ⓘ 弹窗）以 `/api/app-config` 的 `quota_help` 为准，客户端不保留本地默认，从未拉到配置就不显示入口；落后的本地数字就是错误信息。改说明先改服务端（`github-ai-trending-api/src/lib/quota-help.js`，数字从常量拼出）。取舍见后端 `docs/quota-help.md`。1.7.0 的生图 Pro 弹窗曾把「每张 10 次额度」写进 strings 并随版发出，就是这条只约束账户页时漏掉的第二个出口。
...
- **页面骨架统一**：所有页面用 `whl.trending.ai.ui.common` 的 `TrendingScaffold` + `TrendingTopAppBar`，**不直接用 `Scaffold` + `TopAppBar`**。
  ...
  - 效果：**顶栏底色恒定为 `surfaceContainer`**，比内容区（`background`）深一档，靠这层固定的层次差把顶栏和内容分开——**不随滚动变色**。全 app 统一为 `pinnedScrollBehavior`——顶栏不走 M3 折叠。**唯一例外是首页的沉浸式浏览**（设置项，默认开）：开启时首页顶栏/子 tab 行/底栏随滚动经 `translationY` 退场（`ui/home/HomeImmersive.kt`，不改测量高度、内容不重排），与 M3 折叠机制无关；二级页不受影响。
